...
def auto_login(username, password):
    ...
    try:
        ...
        try:
            username_field = browser.wait_for("input[name='username'], input[type='text'], #username", timeout=3)
            username_field.clear()
            username_field.send_keys(username)
            
            password_field = browser.find("input[name='password'], input[type='password'], #password")
            ...
            try:
                submit_btn = browser.find("button[type='submit'], input[type='submit'], button:contains('Login'), button:contains('Sign in')")
